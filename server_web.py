...
pygame.init()
engine = create_engine("postgresql+psycopg2://postgres:96560517bd@localhost/baza_ball")
...
def find(vector: str):
    ...
    for num, sign in enumerate(vector):
        ...
        if sign == ">" and first is not None:
            second = num
            result = vector[first + 1:second]  # Убрали split и map
            return result
    return ""
    ...
    for id in list(players):
        if players[id].sock is not None:
            try:
                ...
                print(f"Ошибка отправки игроку {id}: {e}")
                players[id].sock.close()
                if id in players:
                    del players[id]
                s.query(Player).filter(Player.id == id).delete()
                s.commit()
                print("Сокет закрыт и игрок удален")
